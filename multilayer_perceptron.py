...
class MultilayerPerceptron:
    ...
    def calculate_dot_product(self,weights,features,bias):
        return (np.dot(weights,features)+bias)

    def backpropagation(self,X,output):
        error_output = cross_entropy(output,self.output_layer_neurons)
        loss_values = -1 *error_output*self.hidden_activation(self.output_layer_neurons,True)

        for i in range(self.n_hidden):
            for j in range(self.output_layer_neurons.shape[1]):
                self._o_weights -= (self.learning_rate*loss_values[0][j]*self.hidden_layer_neurons[0][i])
        
        hidden_output = np.matmul(self._o_weights,loss_values.reshape(loss_values.shape[1],1))
        
        for i in range(self.output_layer_neurons.shape[1]):
            for j in range(self.n_hidden):
                self._h_weights -= (self.learning_rate * hidden_output[j] * X[i]) 
    ...
    def fit(self, X, y):
        ...
        self._h_weights = self.initialize_weights(self._X.shape[1],self.n_hidden)
        self._o_weights = self.initialize_weights(self.n_hidden,len(np.unique(y)))
        i=0
        for feature in self._X:
            squared_error = 0
            for iteration_count in range(self.n_iterations):
                self.hidden_layer_neurons = self.hidden_activation(self.calculate_dot_product(feature.reshape(1,len(feature)),self._h_weights,self._h_bias))
                self.output_layer_neurons = self._output_activation(self.hidden_activation(self.calculate_dot_product(self.hidden_layer_neurons,self._o_weights,self._o_bias)))
                if np.absolute(squared_error - np.sum(np.square(self._y[i]-self.output_layer_neurons)))<0.1:
                    break
                squared_error = np.sum(np.square(self._y[i]-self.output_layer_neurons))
                if iteration_count%20 == 0: 
                    self._loss_history.append(squared_error)
                self.backpropagation(feature,self._y[i])
            i += 1
    ...
    def predict(self, X):
        ...
        my_predictions = []
        for i in X:
            feed_forward = self.hidden_activation(self.calculate_dot_product(i.reshape(1,len(i)),self._h_weights,self._h_bias))
            feed_forward = self._output_activation(self.hidden_activation(self.calculate_dot_product(feed_forward,self._o_weights,self._o_bias)))
            my_predictions.append(np.argmax(feed_forward,axis=1)[0])
